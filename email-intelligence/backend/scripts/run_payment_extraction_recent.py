"""Batch payment extraction for recent Gmail mail.

Usage (from email-intelligence/backend):
  ./.venv/bin/python scripts/run_payment_extraction_recent.py --days 12
...
from app.repository.payment_metadata_repository import list_messages_received_since, upsert_message_payment_metadata
from app.settings import Settings


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Extract payment metadata from recent emails")
    p.add_argument("--days", type=int, default=12, help="Only process messages newer than this many days")
    p.add_argument(
        "--max-messages",
        type=int,
        default=None,
...
def main() -> int:
    ...
    # Step 2: Extract payments for the recent DB slice.
